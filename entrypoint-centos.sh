...
HTTPD_ENV_FILE=/etc/httpd/envvars
...
echo "export DOCKER_REGISTRY_HOST=$ENV_DOCKER_REGISTRY_HOST" >> $HTTPD_ENV_FILE
echo "export DOCKER_REGISTRY_PORT=$ENV_DOCKER_REGISTRY_PORT" >> $HTTPD_ENV_FILE
echo "export REGISTRY_PROXY_FQDN=$ENV_REGISTRY_PROXY_FQDN" >> $HTTPD_ENV_FILE
echo "export REGISTRY_PROXY_PORT=$ENV_REGISTRY_PROXY_PORT" >> $HTTPD_ENV_FILE

needModSsl=0
if [ -n "$ENV_DOCKER_REGISTRY_USE_SSL" ]; then
  echo "export DOCKER_REGISTRY_SCHEME=https" >> $HTTPD_ENV_FILE
  needModSsl=1
else
  echo "export DOCKER_REGISTRY_SCHEME=http" >> $HTTPD_ENV_FILE
fi
...
if [ "$ENV_MODE_BROWSE_ONLY" == "true" ]; then
  echo "export APACHE_ARGUMENTS='-D FRONTEND_BROWSE_ONLY_MODE'" >> $HTTPD_ENV_FILE
fi
...
if [ -n "$ENV_AUTH_USE_KERBEROS" ]; then
  ...
  echo "export USE_KERBEROS_AUTH=$ENV_AUTH_USE_KERBEROS" >> $HTTPD_ENV_FILE
  echo "export AUTH_NAME=\"$ENV_AUTH_NAME\"" >> $HTTPD_ENV_FILE
  echo "export AUTH_KRB5_KEYTAB=$ENV_AUTH_KRB5_KEYTAB" >> $HTTPD_ENV_FILE
  echo "export AUTH_KRB_REALMS=$ENV_AUTH_KRB_REALMS" >> $HTTPD_ENV_FILE
  echo "export AUTH_KRB_SERVICE_NAME=$ENV_AUTH_KRB_SERVICE_NAME" >> $HTTPD_ENV_FILE
fi
